...
{ 
    int marco, victima; 
    marco=S->tdm[S->listaocupados].sig;//primer marco
    victima = S->tdm[marco].pagina;
    S->listaocupados = marco;//apunto al ultimo marco introducido
...
void mostrar_informe_reemplazo(ssistema *S)
{
    ...
    int marco;
    printf("%5s %5s %5s \n","marco","pagina","siguiente");
    for(int i=0;i < S->nummarcos;i++){
        marco = (S->listaocupados+i)%S->nummarcos;
        printf("%6d %6d %6d \n",marco,S->tdm[marco].pagina,S->tdm[marco].sig);
