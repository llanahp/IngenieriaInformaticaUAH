...
object Principal {
  ...
  def rellenarTablero(tablero: List[Int], nivel: Int, posicion: Int): List[Int] = {
    if (posicion > (maxFilas(nivel) * maxColumnas(nivel))) {
      tablero
    } else {
  //println(posicion)
      if (getElemento(posicion, tablero) == 0) {
        if (nivel == 1) {
          val aleatorio = numeroAleatorio(1, 3)
          rellenarTablero(setValor(aleatorio, posicion, tablero),nivel, posicion+1)
        } else if (nivel == 2) {
          val aleatorio = numeroAleatorio(1, 5)
          rellenarTablero(setValor(aleatorio, posicion, tablero),nivel, posicion+1)

        } else {
          val aleatorio = numeroAleatorio(1, 7)
          rellenarTablero(setValor(aleatorio, posicion, tablero),nivel, posicion+1)

        }

      } else {
        rellenarTablero(tablero, nivel, (posicion + 1))
      }
    }
  ...
  def recolocarTabler(tablero: List[Int], nivel: Int): List[Int] = {
    ...
    val table = rellenarTablero(tab, nivel, 1)
    ...
    if (seguirJugando(vidas, tiempoInicial) && quedanBloques(tablero)) { //El juegador puede seguir jugando
      if (jugador == 1) { //Juega la persona
        ...
        if (getElemento(posicion, tablero) == 8) { //Es Bomba
          ...
          val tab = recolocarTabler(table, nivel)
          imprimir(tab, nivel)
          val posi = getPosicionJugar(1, 0, 0, tab, nivel)
      ...
      if (Duration(System.nanoTime() - tiempoInicial, NANOSECONDS).toSeconds < 300) { //suficientes vidas y no han pasado 5 minutos que son 300 segundos
        ...
        if (jugador == 1) //Juega la persona
          ...
          val posicionsiguiente = getPosicionJugar(1, 0, 0, tableroSiguiente, nivelSiguiente)
  ...
  def getPosicionJugar(posicionRecorrido: Int, mejorPosi: Int, quitaMejor: Int, tablero: List[Int], nivel: Int): Int = {
    if (posicionRecorrido > maxFilas(nivel) * maxColumnas(nivel)) { //he recorrido el tablerro
      ...
      if (quitaMejor <= cuantosBorraria(posicionRecorrido, tablero, nivel)) //Encuentro una posicion ,mejor
        return getPosicionJugar(posicionRecorrido + 1, posicionRecorrido, cuantosBorraria(posicionRecorrido, tablero, nivel), tablero, nivel)
