...
object Acciones {
  ...
  def getColumna(columna: Int, tablero: ParVector[Int], nivel: Int): ParVector[Int] = {
    if (columna <= (maxColumnas(nivel) * maxFilas(nivel))) { //si no he llegado al final del tablero
      val elementoCola = getColumna((columna + maxColumnas(nivel)), tablero, nivel).toList
      return (getElemento(columna, tablero) :: elementoCola).toVector.par
    } else { //llego al final del tablero
      return (Vector.empty).par //TODO Puede causar problemas
    }
  }

  //
  def reemplazarColumna(columna: Int, tablero: ParVector[Int], nivel: Int): ParVector[Int] = {
  ...
  def longitud(l: ParVector[Int]): Int = {
    if (!l.isEmpty) {
      1 + longitud(l.tail)
    } else {
      0
  ...
  def setValor(valor: Int, pos: Int, l: ParVector[Int]): ParVector[Int] = {
    if (l.isEmpty) {
      (Vector.empty).par
    }
    else if (pos == 1) {
      val elementoCola = l.tail.toList
      (valor :: elementoCola).toVector.par
    }
    else {
      val elementoCola = setValor(valor, (pos - 1), l.tail).toList
      (l.head :: elementoCola).toVector.par
    }
  }

  //Me retorna el elmento que se encuentre en la posición indicada del tablero
  def getElemento(n: Int, l: ParVector[Int]): Int = n - 1 match {
    case 0 => l.head
  ...
  def borrarArriba(posicionArriba: Int, target: Int, tablero: ParVector[Int], nivel: Int): ParVector[Int] = {
  ...
  def borrarAbajo(posicionAbajo: Int, target: Int, tablero: ParVector[Int], nivel: Int): ParVector[Int] = {
  ...
  def borrarDerecha(target: Int, posicionDerecha: Int, tablero: ParVector[Int], nivel: Int): ParVector[Int] = {
  ...
  def borrarIzquieda(posicionIzquiedo: Int, target: Int, tablero: ParVector[Int], nivel: Int): ParVector[Int] = {
