...
object Principal {
  ...
  def crearTablero(filas: Int, columnas: Int, colores: Int, tablero: ParVector[Int]): ParVector[Int] = {
    if (filas * columnas != longitud(tablero)) {
      val a = numeroAleatorio(1, colores)
      val elementoCola = (tablero).toList

      return crearTablero(filas, columnas, colores, (a :: elementoCola).toVector.par)
    } else
      return tablero
  }

  //en función del nivel, pone las bombas por encima del tablero creado
  def crearBombas(tablero: ParVector[Int], nivel: Int): ParVector[Int] = nivel match {
  ...
  def crearBombasAux(tablero: ParVector[Int], bombas: Int): ParVector[Int] = {
    if (bombas > 0) {
  ...
  def crearTipoTablero(nivel: Int): ParVector[Int] = nivel match {
    case 1 => crearTablero(9, 11, 3, (Vector.empty).par) //2 bombas
    case 2 => crearTablero(12, 16, 5, (Vector.empty).par) //3 bombas
    case 3 => crearTablero(25, 15, 7, (Vector.empty).par) //5 bombas
  }
  ...
  def tiene3EnLinea(posicion: Int, tablero: ParVector[Int], nivel: Int): Boolean = {
  ...
  def puntos3EnLinea(posicion: Int, tablero: ParVector[Int], nivel: Int): Int = {
  ...
  def eliminar3EnLinea(posicion: Int, tablero: ParVector[Int], nivel: Int): ParVector[Int] = {
    if (estaEnBordes(posicion, maxFilas(nivel), maxColumnas(nivel))) {
      val filas = maxFilas(nivel)
      val columnas = maxColumnas(nivel)
      //miro si es una esquina
  ...
  def eliminarBloquesBomba(posicion: Int, tablero: ParVector[Int], nivel: Int): ParVector[Int] = {
    val filas = maxFilas(nivel)
    val columnas = maxColumnas(nivel)
  ...
  def recorrerTablero(posicion: Int, tablero: ParVector[Int], nivel: Int): ParVector[Int] = {
  ...
  def dejarCaer(tablero: ParVector[Int], nivel: Int): ParVector[Int] = {
  ...
  def moverIzquierda(columna: Int, tablero: ParVector[Int], nivel: Int): ParVector[Int] = {
  ...
  def recolocarTabler(tablero: ParVector[Int], nivel: Int): ParVector[Int] = {
  ...
  def jugarPosicion(posicion: Int, tablero: ParVector[Int], nivel: Int, bloquesEliminados: Int, vidas: Int, puntuacion: Int, tiempoInicial: Long, jugador: Int, partidasJugadas: Int): Unit = {
    if (seguirJugando(vidas, tiempoInicial) && quedanBloques(tablero)) { //El juegador puede seguir jugando
      if (jugador == 1) { //Juega la persona
        ...
        if (getElemento(posicion, tablero) == 8) { //Es Bomba
          ...
          val tab = recolocarTabler(table, nivel)
          imprimir(tab, nivel)
          val posi = getPosicionJugar(1, 0, 0, tab.toVector.par, nivel)
      ...
      if (Duration(System.nanoTime() - tiempoInicial, NANOSECONDS).toSeconds < 300) { //suficientes vidas y no han pasado 5 minutos que son 300 segundos
        ...
        if (jugador == 1) //Juega la persona
          ...
          val posicionsiguiente = getPosicionJugar(1, 0, 0, tableroSiguiente.toVector.par, nivelSiguiente)
  ...
  def cuantosBorraria(posicion: Int, tablero: ParVector[Int], nivel: Int): Int = {
    if (posicion > maxFilas(nivel) * maxColumnas(nivel)) { //si excedo los límites
      0
    } else {
      if (getElemento(posicion, tablero.toVector.par) == 8) { //Estamos ante una bomba
        return (puntosBomba(posicion, tablero.toVector.par, nivel) / 10)
      } else {
        if (tiene3EnLinea(posicion, tablero.toVector.par, nivel)) { //tiene mínimo 3 en linea
          return (puntos3EnLinea(posicion, tablero.toVector.par, nivel))
        } else { //solo puedo borrar el propio bloque
          if (getElemento(posicion, tablero.toVector.par) == 0) { //es una posicion con un espacio vacio
            0
  ...
  def getPosicionJugar(posicionRecorrido: Int, mejorPosi: Int, quitaMejor: Int, tablero: ParVector[Int], nivel: Int): Int = {
    if (posicionRecorrido > maxFilas(nivel) * maxColumnas(nivel)) { //he recorrido el tablerro
      if (quitaMejor == 1) { //si solo puedo borrar 1 bloque
        return ((maxFilas(nivel) * maxColumnas(nivel)) - maxColumnas(nivel) + 1) //elimino el de abajo a la izquierd
      } else {
        return mejorPosi //elimino el que pueda borrar más bloques
      }
    } else { //Sigo recorriendo el tablero mirando si existen posiciones que borren más bloques
      if (quitaMejor <= cuantosBorraria(posicionRecorrido, tablero.toVector.par, nivel)) //Encuentro una posicion ,mejor
        return getPosicionJugar(posicionRecorrido + 1, posicionRecorrido, cuantosBorraria(posicionRecorrido, tablero.toVector.par, nivel), tablero, nivel)
      else //no es mejor que otra ya encontrada
        return getPosicionJugar(posicionRecorrido + 1, mejorPosi, quitaMejor, tablero, nivel)
    }
  }
  ...
  def main(args: Array[String]) {
    ...
    val tablero = crearBombas(crearTipoTablero(nivel), nivel).toVector.par
    //muestro el tablero
    ...
    if (jugador == 1) //Juega la persona
      ...
      val posi = getPosicionJugar(1, 0, 0, tablero, nivel)
      maquinajuegaPoscion(posi, nivel)
      espera(2000)
